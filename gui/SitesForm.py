...
class SitesDialog ( wx.Dialog ):
    ...
    def __init__(self, parent, sites, item):
        wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = "Select Sites", size= wx.Size(400,300))
    ...
    def OnOk(self, event):
        ...
        if len(sites) > 0:
            pub.sendMessage(EVENTS.ITEM_COPIED, item=cpy.deepcopy(self._item), sites=sites)
            self.Close()
        else:
            wx.MessageBox(ERR.ERRORS[ERR.NO_SITE_SEL], 'Error', wx.OK|wx.ICON_ERROR)