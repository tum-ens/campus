...
class TransmissionDialog ( bf.BasicForm ):
    ...
    def CreateGeneralLayout(self):
        ...
        label = wx.StaticText(self, -1, "Transmission name:", size=wx.Size(120, -1))
    ...
    def OnSiteChange(self, event):
        ...
        if not self._ddlSiteIn.GetValue() or not self._ddlSiteOut.GetValue():            
            ...
            wx.MessageBox('Site out should not be the same as site in.', 'Error', wx.OK|wx.ICON_ERROR)
            return
            
        l = self._controller.GetCommonCommodities(self._ddlSiteIn.GetValue(), 
                                                  self._ddlSiteOut.GetValue())
        
        if len(l) > 0:
            self._ddlComm.SetItems(l)
        else:
            wx.MessageBox('There is no common commodities between selected sites', 
    ...
    def PopulateTrans(self, trnsm, sitesList):
        ...
        if trnsm['SiteIn'] and trnsm['SiteOut']:
            l = self._controller.GetCommonCommodities(trnsm['SiteIn'], trnsm['SiteOut'])
            self._ddlComm.SetItems(l)
    ...
    def OnOk(self, event):
        if not self._ddlSiteIn.GetValue():            
            wx.MessageBox('Please select a site in.', 'Error', wx.OK|wx.ICON_ERROR)
        elif not self._ddlSiteOut.GetValue():
            wx.MessageBox('Please select a site out.', 'Error', wx.OK|wx.ICON_ERROR)
        elif self._ddlSiteIn.GetValue() == self._ddlSiteOut.GetValue():
            wx.MessageBox('Site out should not be the same as site in.', 'Error', wx.OK|wx.ICON_ERROR)
